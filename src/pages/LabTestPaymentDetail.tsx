...
const LabTestPaymentDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMedicalBillDetails = async () => {
      ...
      setLoading(true);
      try {
        const response = await apiService.get<MedicalBill>(`/medical-bills/${id}`);
  ...
  if (!medicalBill) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-[#f7f7f7]">
        <h2 className="text-2xl font-bold text-[#000000] mb-4">Medical Bill Not Found</h2>
        <p className="text-gray-600 mb-8">The medical bill you're looking for does not exist or has been removed.</p>
        <button 
  ...
  const paidTests = medicalBill.examinationDetails.filter(test => test.status === 'PAID').length;
    ...
    <div className="w-full min-h-screen bg-[#f7f7f7] pb-20">
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="flex flex-col my-5 justify-center items-center">
          <h1 className="text-4xl font-bold font-sans my-5 text-[#000000]">LAB TEST DETAILS</h1>
        </div>

        {/* Patient Information */}
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          <h2 className="text-2xl font-bold text-[#000000] mb-6 border-b pb-2">Patient Information</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-[#000000]">{medicalBill.patientName}</p>
              </div>
            </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-[#000000]">{medicalBill.patientGender}</p>
              </div>
            </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-[#000000]">{medicalBill.doctorName}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          <div className="flex justify-between items-center mb-6 border-b pb-2">
            <div>
              ...
              <div className="flex mt-2 text-sm space-x-4">
                <span className="text-gray-500">Total: {medicalBill.examinationDetails.length}</span>
                {paidTests > 0 && (
            ...
            <div className="flex items-center bg-[#edf9f2] px-4 py-2 rounded-lg">
              <p className="text-[#34a85a] font-medium mr-2">Total Unpaid Amount:</p>
              <p className="text-2xl font-bold text-[#34a85a]">{formatCurrency(medicalBill.totalUnpaidAmount)}</p>
            </div>
          </div>
          ...
          {medicalBill.examinationDetails && medicalBill.examinationDetails.length > 0 ? (
            <div className="space-y-4">
              {medicalBill.examinationDetails.map((test) => (
                <div 
                  ...
                  onClick={() => test.status !== 'PAID' && handleLabTestToggle(test.id)}
                >
                  <div className="flex items-center">
                    {test.status !== 'PAID' && (
                    ...
                    <p className="text-lg font-medium text-[#000000]">{test.examinationType}</p>
                  </div>
                  <div className="flex items-center">
                    <div className="mr-6">
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">No lab tests found for this medical bill.</p>
            </div>
          )}
        </div>
      </div>
