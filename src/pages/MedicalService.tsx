...
const MedicalService: React.FC = () => {
  ...
  const [medicalBillData, setMedicalBillData] = useState<MedicalBillResponseDTO | null>(null);
  const {
    patientInfo,
    syndrome,
    note,
  ...
  useEffect(() => {
    const fetchMedicalBillData = async () => {
      try {
        ...
        const medicalBillId = new URLSearchParams(window.location.search).get('id');
        if (medicalBillId) {
          const response = await apiService.get<MedicalBillResponseDTO>(`/medical-bills/${medicalBillId}`);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center min-w-[200px] mx-16">
            ...
            <div>
              ...
              <p className="text-xl text-gray-700">
                {medicalBillData ? 
                  new Date(medicalBillData.patientBirthDate).toLocaleDateString() :
          ...
          {/* View History Button */}
          <button
            onClick={() => window.open(`/medical-history?id=${medicalBillData?.patientId || patientInfo.patientId}`, '_blank')}
      ...
      <div className="mb-12">
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-sm p-8">
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-800 mb-6">Pre-Examination Results</h3>
            <div className="grid grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-gray-500">Weight</p>
                <p className="text-lg font-medium">{medicalBillData?.weight} kg</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Heart Rate</p>
                <p className="text-lg font-medium">{medicalBillData?.heartRate} bpm</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Blood Pressure</p>
                <p className="text-lg font-medium">{medicalBillData?.bloodPressure}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Temperature</p>
                <p className="text-lg font-medium">{medicalBillData?.temperature} °C</p>
              </div>
            </div>
          </div>
        </div>
