...
const Schedule: React.FC = () => {
  ...
  const handlePatientClick = async (appointment: Appointment) => {
    if (
      ...
      appointment.status === "lab_test_completed"
    ) {
      try {
        ...
        const medicalBill = await apiService.get<MedicalBillResponse>(
      ...
      switch (appointment.status.toLowerCase()) {
        case "success":
          toast.info(
            `This appointment has already been completed for ${appointment.patientName}`
          );
          break;
        case "cancelled":
          toast.warning(
            `This appointment was cancelled for ${appointment.patientName}`
          );
          break;
        ...
        default:
          toast.error(
            `Appointment must be pre-examined to proceed with medical service.`
          );
      }
    }
  };
  ...
  const handleStatusChange = (index: number, newStatus: StatusType) => {
    ...
    if (appointment) {
      dispatch(
        updateAppointmentStatus({ id: appointment.id, status: newStatus })
      )
        .unwrap()
        .then(() => {
          switch (newStatus.toLowerCase()) {
            case "success":
              toast.success(
                `Appointment completed successfully for ${appointment.patientName}`
              );
              break;
            case "cancelled":
              toast.info(
            ...
            case "lab_test_completed":
              toast.success(
                `Lab tests completed for ${appointment.patientName}`
              );
              break;
            default:
              toast.info(
                `Appointment status updated to ${newStatus} for ${appointment.patientName}`
              );
          }
        })
        .catch((error) => {
    ...
    <div className='w-full min-h-screen bg-gray-50'>
      ...
      <div className='my-6 sm:my-8 md:my-12 flex flex-col items-center px-4 sm:px-6 md:px-8'>
        ...
        {loading ? (
          ...
          <div className='text-red-500 text-center p-3 sm:p-4'>{error}</div>
        ) : (
          <div className='flex flex-col items-center w-full'>
            {sortedAppointments.length === 0 ? (
              ...
              <>
                <div className='cursor-pointer w-full sm:w-10/12 md:w-9/12 space-y-3 sm:space-y-4 max-h-[60vh] sm:max-h-[65vh] md:max-h-[70vh] pr-2 sm:pr-3 md:pr-4 overflow-y-auto'>
                  {sortedAppointments.map((appointment, index) => (
                    <AppointmentCard
                      ...
                      appointment={{
                        ...
                        birthDate:
                          appointment.patientResponseDTO?.birthDate || "",
                      }}
                      index={index}
